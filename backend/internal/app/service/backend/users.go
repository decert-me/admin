...
func GetUsersList(r request.GetUsersListReq) (label []response.GetUsersListRes, total int64, err error) {
	...
	db := global.DB.Table("users").
		Select("users.id as user_id, users.address, users.nickname, string_agg(tag.name, ',') as tags, to_timestamp(users.creation_timestamp) as created_at").
...
func GetUsersInfo(r request.GetUsersInfoReq) (label response.GetUsersInfoRes, err error) {
	...
	if err := global.DB.Table("users").
		Select("id as user_id, address, nickname, to_timestamp(creation_timestamp) as created_at").
		Where("id = ?", r.UserID).
		Limit(1).
